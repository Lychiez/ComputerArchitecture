;; mic1 program that will input 2 positive numbers,
;; add them, and print either result/overflow using
;; the CSR registers
main:	loco result:		; load the addr. of the result to
	push			; get back from AddInput, then call
	call addinp:		; the function
	halt
...
twoinp:	pop
	halt
	stod num2:
sumofd: loco sumstr:		; load the string that says sum ints
nextw2: pshi			; push first 2 chars on stack
        addd c1:		; get next 2 chars in string
        stod pprmpt:
        pop
        jzer crnl2:		; if null byte, done
        stod 4094		; put first char in xmtr
        push
        subd c255:		; get next char in 2 byte seq.
        jneg crnl2:		; if null byte, done
        call sb:		; swap bytes in string
        insp 1
        push			; push new string on
        call xbsywt:		; wait for xmtr
...
crnl2:  lodd cr:		; print carriage return
        stod 4094		
        call xbsywt:
        lodd nl:		; print new line
        stod 4094
...
nextw3: pshi			; push first 2 chars on stack
        ...
        stod pprmpt:
        pop
        jzer crnl3:		; if null byte, done
        stod 4094		; put first char in xmtr
        push
        subd c255:		; get next char in 2 byte seq.
        jneg crnl3:		; if null byte, done
        call sb:		; swap bytes in string
        insp 1
        push			; push new string on
        call xbsywt:		; wait for xmtr
...
crnl3:  lodd cr:		; print carriage return
        stod 4094		
        call xbsywt:
        lodd nl:		; print new line
        stod 4094
